...
void MasternodeManager::on_localButton_clicked()
{
    ...
    if(pwalletMain->GetBalance() < 250000.1*COIN)
    {
	QMessageBox msg;
        msg.setText("You must have at least 250000.1 GMCS to cover the 250000 GMCS collateral for a Masternode and the tx fee.");
	msg.exec();
	return;
    }
    ...
    {
	QMessageBox msg;
        msg.setText("Your wallet must be unlocked so that the 250000 GMCS collateral can be sent.");
	msg.exec();
	return;
    }
    ...
    string strError = pwalletMain->SendMoneyToDestination(CBitcoinAddress(account.vchPubKey.GetID()).Get(), 250000*COIN, sNarr, wtx);
    if (strError != "")
    {
	QMessageBox msg;
    ...
    {
	QMessageBox msg;
	std::string sMsg = "Local Masternode created and 250000 GMCS sent to the collateral address.  Transaction hash:\n";
