...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Gridmaster"))
        return false;
...
{
    // Convert Gridmaster:// to Gridmaster:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Gridmaster://"))
    {
        uri.replace(0, 12, "Gridmaster:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Gridmaster.lnk";
}
...
{
    return GetAutostartDir() / "Gridmaster.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Gridmaster-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Gridmaster-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Gridmaster-Qt"));
