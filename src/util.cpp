...
int nAnonymizeGridmasterAmount = 500;
...
#else
    const char* pszModule = "Gridmaster";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Gridmaster
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Gridmaster
    // Mac: ~/Library/Application Support/Gridmaster
    // Unix: ~/.Gridmaster
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gridmaster";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Gridmaster.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Gridmasterd.pid"));
